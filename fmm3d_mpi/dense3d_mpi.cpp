...
int Dense3d_MPI::setup()
{
	...
	* See http://www-unix.mcs.anl.gov/petsc/petsc-as/snapshots/petsc-current/docs/manualpages/Vec/VecScatterCreateToAll.html */
  {
	 ...
	 pC( VecScatterBegin(_srcPos, _srcAllPos, INSERT_VALUES, SCATTER_FORWARD, ctx) );
	 pC( VecScatterEnd(  _srcPos, _srcAllPos, INSERT_VALUES, SCATTER_FORWARD, ctx) );
  ...
  /* Create a Scatter context - copies all source normal values to each processor */
  {
	 ...
	 pC( VecScatterBegin(_srcNor, _srcAllNor, INSERT_VALUES, SCATTER_FORWARD, ctx) );
	 pC( VecScatterEnd(  _srcNor, _srcAllNor, INSERT_VALUES, SCATTER_FORWARD, ctx) );
	 pC( VecScatterDestroy(ctx) );
  }
  return(0);
}
...
int Dense3d_MPI::evaluate(Vec srcDen, Vec trgVal) 
{
  ...
  /* Create scatter context to scatter source densities to all processors */
  {
	 ...
	 pC( VecScatterBegin(srcDen, srcAllDen, INSERT_VALUES, SCATTER_FORWARD, ctx) );
	 pC( VecScatterEnd(  srcDen, srcAllDen, INSERT_VALUES, SCATTER_FORWARD, ctx) );
	 pC( VecScatterDestroy(ctx) );
  }
